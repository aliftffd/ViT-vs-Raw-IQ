...
def suggest_safe_hyperparameters(trial):
    ...
    # === BATCH SIZE ===
    # Proven working: batch_size=128
    # Adjust based on model size to stay within VRAM limits
    
    if d_model <= 192:
        # Smaller models: can use larger batches
        batch_size = trial.suggest_categorical("batch_size", [96, 128, 160])
    elif d_model == 256:
        # Proven configuration uses 128 successfully
        batch_size = trial.suggest_categorical("batch_size", [96, 128, 144])
    else:  # d_model >= 320
        # Larger models: use moderate batches
        batch_size = trial.suggest_categorical("batch_size", [64, 96, 128])
    
    # Memory safety check (but less conservative than before)
    is_safe, estimated_vram = get_memory_safe_constraints(d_model, n_layers, batch_size)
    
    # Only prune if REALLY unsafe (>14GB estimated)
    # We know d_model=256, n_layers=9, batch=128 works fine
    if estimated_vram > 14.0:
        print(f"Trial {trial.number}: High VRAM estimate ({estimated_vram:.1f}GB) - pruning for safety")
        raise optuna.exceptions.TrialPruned()
